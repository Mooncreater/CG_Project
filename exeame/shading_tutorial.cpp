...
void ShadingTutorial::initPhongShader() {
    ...
    // Phong fragment shader: ambient + diffuse + specular (Blinn/Phong-like)
    const char* fsCode =
        "#version 330 core\n"
        "in vec3 fPosition;\n"
        "in vec3 fNormal;\n"
        ...
        "    vec3 specular = directionalLight.color * spec * material.ks;\n"
        ...
        "        return vec3(0.0);\n"
        "    }\n"
        "    vec3 diffuse = spotLight.color * max(dot(lightDir, normal), 0.0) * material.kd;\n"
        "    vec3 reflectDir = reflect(-lightDir, normal);\n"
        "    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.ns);\n"
        ...
        "    vec3 ambient = material.ka * ambientLight.color * ambientLight.intensity;\n"
        "    vec3 result = ambient + calcDirectionalLight(normal, viewDir) + calcSpotLight(normal, viewDir);\n"
