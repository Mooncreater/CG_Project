...
struct Demo : Application {
    ...
    void renderFrame() override {
        ...
        if (first) {
            std::cout << "Geometry: " << _mesh->indexCount() / 3 << " triangles\n";
            first = false;
        }
        ...
        ImGui::Text("Geometry: %d tris", _mesh->indexCount() / 3);
...
int main() {
    Options option;
    option.windowTitle = "09 Humanoid Debug";
    option.assetRootDir = "media";
    option.windowWidth = 1024;
    option.windowHeight = 768;
    option.glVersion = { 3,3 };
