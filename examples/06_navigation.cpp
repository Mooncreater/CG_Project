...
struct Demo : Application {
    ...
    Demo(const Options& o) :Application(o) {
        ...
        _sphere = m(Primitives::CreateSphere(0.7f, 32));
    ...
    void handleInput() override {
        ...
        if (_input.keyboard.keyStates[GLFW_KEY_F] == GLFW_PRESS) {
            ...
            _fc.target = (mn + mx) * 0.5f;
            _fc.dist = glm::length(mx - mn) * 1.2f;
            _fc.pitch = 20;
    ...
    void renderFrame() override {
        ...
        auto P = glm::perspective(glm::radians(50.f), (float)_windowWidth / _windowHeight, 0.5f, 100.f);
        auto V = _fc.viewMatrix();

        for (int i = -10; i <= 10; i++) {
            drawLit(_s, _cube.get(), glm::scale(glm::translate(glm::mat4(1), glm::vec3(i, 0, 0)), glm::vec3(0.02f, 0.01f, 20)), glm::vec3(0.3f), P, V);
        ...
        for (size_t i = 0; i < _pos.size(); i++)
            drawLit(_s, ms[i % 4], glm::translate(glm::mat4(1), _pos[i]), cs[i % 4], P, V);
        ...
        ImGui::Text("R-drag:Orbit M-drag:Pan Scroll:Zoom WASD:Move F:Fit");

        auto cp = _fc.position();
        ImGui::Text("Cam:%.0f,%.0f,%.0f Dist:%.0f", cp.x, cp.y, cp.z, _fc.dist);
        ImGui::End();
