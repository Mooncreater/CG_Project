...
struct Demo : Application {
    ...
    GLuint _sfbo=0, _smap=0; int _ssz=2048;
    bool _shadows=true;
    ...
    Demo(const Options& o):Application(o){
        ...
        const char*vs=R"(#version 330 core
            ...
            out vec3 fn,fp,flp;uniform mat4 uM,uV,uP,uLVP;
            void main(){vec4 w=uM*vec4(aP,1);fp=w.xyz;fn=mat3(transpose(inverse(uM)))*aN;
            vec4 lp=uLVP*w;flp=lp.xyz/lp.w*0.5+0.5;gl_Position=uP*uV*w;})";
        const char*fs=R"(#version 330 core
            in vec3 fn,fp,flp;out vec4 o;uniform vec3 uSD,uSC,uPP,uPC,uOC;uniform float uPI,uAmb,uBias;
            uniform sampler2DShadow uShadow;uniform bool uShadows;
            float shadow(){if(!uShadows)return 1.0;float s=0;vec2 ts=1.0/textureSize(uShadow,0);
            for(int x=-1;x<=1;x++)for(int y=-1;y<=1;y++)s+=texture(uShadow,vec3(flp.xy+vec2(x,y)*ts,flp.z-uBias));return s/9.0;}
            void main(){vec3 N=normalize(fn),L=normalize(uSD);float nd=max(dot(N,L),0.),sh=shadow();
            vec3 tl=uPP-fp;float d=length(tl),att=uPI/(1+d*d*.02);float np=max(dot(N,normalize(tl)),0.);
            o=vec4(uOC*uAmb+uOC*uSC*nd*sh+uOC*uPC*np*att,1);})";
        _shader=std::make_unique<GLSLProgram>();_shader->attachVertexShader(vs);_shader->attachFragmentShader(fs);_shader->link();

        // Depth shader
        const char*dvs=R"(#version 330 core
layout(location=0) in vec3 aP;
uniform mat4 uM,uLVP;
void main(){gl_Position=uLVP*uM*vec4(aP,1);}
)";
        const char*dfs=R"(#version 330 core
void main(){}
)";
        _shadowS=std::make_unique<GLSLProgram>();_shadowS->attachVertexShader(dvs);_shadowS->attachFragmentShader(dfs);_shadowS->link();

        // Shadow map
        glGenFramebuffers(1,&_sfbo);glGenTextures(1,&_smap);
        glBindTexture(GL_TEXTURE_2D,_smap);glTexImage2D(GL_TEXTURE_2D,0,GL_DEPTH_COMPONENT,_ssz,_ssz,0,GL_DEPTH_COMPONENT,GL_FLOAT,0);
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,GL_CLAMP_TO_BORDER);
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_CLAMP_TO_BORDER);
        float bd[]={1,1,1,1};glTexParameterfv(GL_TEXTURE_2D,GL_TEXTURE_BORDER_COLOR,bd);
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_COMPARE_MODE,GL_COMPARE_REF_TO_TEXTURE);
        glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_COMPARE_FUNC,GL_LEQUAL);
        glBindFramebuffer(GL_FRAMEBUFFER,_sfbo);glFramebufferTexture2D(GL_FRAMEBUFFER,GL_DEPTH_ATTACHMENT,GL_TEXTURE_2D,_smap,0);
        ...
        _sphere=m(Primitives::CreateSphere(0.5f,36));_plane=m(Primitives::CreateCube(1));
        _cube=m(Primitives::CreateCube(1));_cyl=m(Primitives::CreateCylinder(0.6f,2,32));
        _cone=m(Primitives::CreateCone(0.7f,1.5f,32));

        _fc.target=glm::vec3(0,1.5f,0);_fc.dist=10;_fc.pitch=25;
    }

    ~Demo(){if(_sfbo)glDeleteFramebuffers(1,&_sfbo);if(_smap)glDeleteTextures(1,&_smap);
    ...
    void shadowPass(glm::mat4 lVP){
        glViewport(0,0,_ssz,_ssz);glBindFramebuffer(GL_FRAMEBUFFER,_sfbo);glClear(GL_DEPTH_BUFFER_BIT);
        glEnable(GL_DEPTH_TEST);glCullFace(GL_FRONT);
        _shadowS->use();_shadowS->setUniformMat4("uLVP",lVP);
        auto draw=[&](Mesh*m,glm::mat4 model){_shadowS->setUniformMat4("uM",model);m->draw();};
    ...
    template<typename D> void drawSceneA_Shadow(D& draw){
        draw(_plane.get(),glm::scale(glm::translate(glm::mat4(1),glm::vec3(0,-2,0)),glm::vec3(10,0.2f,10)));//floor
        draw(_plane.get(),glm::scale(glm::translate(glm::mat4(1),glm::vec3(0,1,-5)),glm::vec3(10,6,0.2f)));//back wall
        draw(_plane.get(),glm::scale(glm::translate(glm::mat4(1),glm::vec3(-5,1,0)),glm::vec3(0.2f,6,10)));//left wall
        draw(_sphere.get(),glm::translate(glm::mat4(1),glm::vec3(-1.5f,-0.8f,-1)));//ball on floor
        draw(_cube.get(),glm::scale(glm::translate(glm::mat4(1),glm::vec3(1.5f,-0.5f,-2)),glm::vec3(1.2f)));//box
    }
    ...
    template<typename D> void drawSceneB_Shadow(D& draw){
        ...
        for(int x=-8;x<=8;x+=4)for(int z=-8;z<=8;z+=4){
            draw(_sphere.get(),glm::translate(glm::mat4(1),glm::vec3(x+0.5f,-0.8f,z+0.5f)));
        }
        for(int x=-6;x<=6;x+=6)draw(_cube.get(),glm::scale(glm::translate(glm::mat4(1),glm::vec3(x,0,0)),glm::vec3(1.5f)));
    ...
    template<typename D> void drawSceneB_Color(D& drawObj){
        ...
        for(int x=-6;x<=6;x+=6)drawObj(_cube.get(),glm::scale(glm::translate(glm::mat4(1),glm::vec3(x,0,0)),glm::vec3(1.5f)), glm::vec3(0.8f,0.3f,0.2f));
    ...
    template<typename D> void drawSceneC_Shadow(D& draw){
        ...
        for(int i=0;i<8;i++){float a=i*0.785f;draw(_sphere.get(),glm::translate(glm::mat4(1),glm::vec3(cos(a)*3,0,sin(a)*3)));}
    ...
    template<typename D> void drawSceneC_Color(D& drawObj){
        ...
        for(int i=0;i<8;i++){float a=i*0.785f;drawObj(_sphere.get(),glm::translate(glm::mat4(1),glm::vec3(cos(a)*3,0,sin(a)*3)), glm::vec3(0.3f+0.08f*i,0.5f,0.7f-0.06f*i));}
    ...
    void renderFrame() override {
        ...
        glClearColor(0.08f,0.1f,0.14f,1);glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);glEnable(GL_DEPTH_TEST);
        auto P=glm::perspective(glm::radians(50.f),(float)_windowWidth/_windowHeight,0.5f,50.f);auto V=_fc.viewMatrix();
        _shader->use();_shader->setUniformMat4("uP",P);_shader->setUniformMat4("uV",V);_shader->setUniformMat4("uLVP",lVP);
        _shader->setUniformVec3("uSD",L);_shader->setUniformVec3("uSC",_sc);
        _shader->setUniformVec3("uPP",_pp);_shader->setUniformVec3("uPC",_pc);
        _shader->setUniformFloat("uPI",_pi);_shader->setUniformFloat("uAmb",_amb);_shader->setUniformFloat("uBias",_bias);
        _shader->setUniformBool("uShadows",_shadows);
        glActiveTexture(GL_TEXTURE1);glBindTexture(GL_TEXTURE_2D,_smap);_shader->setUniformInt("uShadow",1);

        auto drawObj=[&](Mesh*m,glm::mat4 model,glm::vec3 col){
            _shader->setUniformMat4("uM",model);_shader->setUniformVec3("uOC",col);m->draw();};
        ...
        ImGui::Text("Scene:");
        for(int i=0;i<3;i++){if(i>0)ImGui::SameLine();ImGui::RadioButton(_sceneNames[i],&_sceneIdx,i);}
        ImGui::Separator();
        ImGui::Checkbox("Shadows",&_shadows);ImGui::SliderFloat("Bias",&_bias,0.0001f,0.05f,"%.4f");
        ImGui::SliderFloat("Ambient",&_amb,0,1);
        ImGui::Text("Sun Dir");ImGui::SliderFloat3("##sd",&_sd[0],-1,1);ImGui::ColorEdit3("Sun Col",&_sc[0]);
        ImGui::Text("Point Pos");ImGui::SliderFloat3("##pp",&_pp[0],-5,5);ImGui::ColorEdit3("Pt Col",&_pc[0]);ImGui::SliderFloat("Pt Int",&_pi,0,20);
