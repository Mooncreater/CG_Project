...
struct Demo : Application {
    ...
    void load() {
        try {
            auto e = ObjLoader::Load(_assetRootDir + "/obj/" + _files[_idx]);
            _mesh = std::make_unique<Mesh>(e.vertices, e.indices);
        }
        catch (...) {}
    }
    ...
    void renderFrame() override {
        ...
        if (_wire)glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
        ...
        ImGui::Begin("OBJ");
        ImGui::Text("File: %s", _files[_idx].c_str());
