...
struct Demo : Application {
    ...
    int _cur = 0, _total = 0;
    float _anim = 0, _speed = 1;
    bool _play = true, _loop = true;
    std::string _status;
    ...
    Demo(const Options& o) :Application(o) {
        ...
        for (auto& f : files)try {
            auto e = ObjLoader::Load(_assetRootDir + "/obj/" + f);
            _frames.push_back(std::make_unique<Mesh>(e.vertices, e.indices));
        }
        catch (...) {}
        ...
        if (_frames.empty()) {
            _frames.push_back(m(Primitives::CreateCube(1)));
            _frames.push_back(m(Primitives::CreateSphere(1, 32)));
            _frames.push_back(m(Primitives::CreateCylinder(1, 1.5f, 32)));
            _frames.push_back(m(Primitives::CreateCone(1, 1.5f, 32)));
        }
        _total = (int)_frames.size();
    ...
    void saveSS() {
        ...
        for (int y = 0;
            y < _windowHeight;
            y++)memcpy(&fl[y * _windowWidth * 3], &px[(_windowHeight - 1 - y) * _windowWidth * 3], _windowWidth * 3);

        char nm[64];
        snprintf(nm, sizeof(nm), "screenshot_%d.png", (int)time(0));

        stbi_write_png(nm, _windowWidth, _windowHeight, 3, fl.data(), _windowWidth * 3);
        _status = "Saved: " + std::string(nm);
    ...
    void handleInput() override {
        ...
        if (_input.keyboard.keyStates[GLFW_KEY_RIGHT] == GLFW_PRESS) {
            _cur = (_cur + 1) % _total;
            _play = false;
        }
        if (_input.keyboard.keyStates[GLFW_KEY_LEFT] == GLFW_PRESS) {
            _cur = (_cur - 1 + _total) % _total;
            _play = false;
        }
    }
    void renderFrame() override {
        glClearColor(0.10f, 0.12f, 0.16f, 1);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        glEnable(GL_DEPTH_TEST);

        if (_play) {
            _anim += _deltaTime * _speed;
            _cur = ((int)(_anim * 5)) % _total;
            if (!_loop && _cur == _total - 1)_play = false;
        }
        ...
        ImGui::Text("Frame:%d/%d", _cur + 1, _total);

        if (ImGui::Button(_play ? "Pause" : "Play"))_play = !_play;
        ImGui::SameLine();
        ...
        ImGui::SliderFloat("Speed", &_speed, 0.1f, 5);
        ImGui::SliderInt("Frame", &_cur, 0, _total - 1);
