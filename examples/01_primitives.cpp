...
struct Demo : Application {
    ...
    Demo(const Options& o) :Application(o) {
        ...
        _m[0] = m(Primitives::CreateCube(1.5f));
        _m[1] = m(Primitives::CreateSphere(1, 36));
        _m[2] = m(Primitives::CreateCylinder(1, 2, 36));
        _m[3] = m(Primitives::CreateCone(1, 2, 36));
        _m[4] = m(Primitives::CreatePrism(6, 1, 2));
        _m[5] = m(Primitives::CreateFrustum(6, 0.6f, 1.2f, 2));
    ...
    void renderFrame() override {
        ...
        for (int i = 0; i < 6; i++)
            if (ImGui::Selectable(_names[i], _sel == i))
                _sel = i;
        ImGui::End();
        ImGui::Render();
