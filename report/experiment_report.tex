...
\author{姓名：\_\_\_\_\_\_\_\_ \quad 学号：\_\_\_\_\_\_\_\_ \quad 班级：\_\_\_\_\_\_\_\_}
\date{\today}
...
基于 Blinn-Phong 模型实现，包含三层光源（完整 shader 见 \texttt{src/render\_util/simple\_lit.h}）：
\[
L = \underbrace{k_a \cdot C \cdot (0.5 + 0.5 N_y)}_{\text{半球环境光}} + \underbrace{S \cdot [k_d \cdot C \cdot (N \cdot L) + k_s \cdot (R \cdot V)^\alpha]}_{\text{平行光(太阳) diffuse+specular}} + \underbrace{\frac{I}{k_c + k_l d + k_q d^2} \cdot C_p \cdot [\text{diffuse} + \text{specular}]}_{\text{点光源}}
\]
...
\begin{itemize}[nosep]
    \item 2048×2044 深度纹理（\texttt{GL\_DEPTH\_COMPONENT}, float）
...
这三种建筑对象的行为差异明确：\textbf{门}可交互开关、\textbf{窗}可穿过可破坏、\textbf{墙}始终阻塞。符合 README 中“如 PUBG 的模型中：关闭的门不可通行，可以按键打开，打开后可通行；窗可打碎、翻越；墙始终不可通行”的要求。
