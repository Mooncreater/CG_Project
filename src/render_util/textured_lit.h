...
// Instanced variant — blocks are axis-aligned, normals don't need transform
class InstancedTexturedLitShader {
    std::unique_ptr<GLSLProgram> _prog;
public:
    InstancedTexturedLitShader() {
        const char* vs = R"(#version 330 core
            layout(location=0) in vec3 aP; layout(location=1) in vec3 aN; layout(location=2) in vec2 aUV;
            layout(location=3) in vec3 aInstancePos;
            out vec3 fn; out vec2 fuv; out vec3 fp; out vec4 fls;
            uniform mat4 uV,uP,uLS;
            void main(){
                vec4 wp=vec4(aP+aInstancePos,1);
                fn=aN;
                fuv=aUV;
                fp=wp.xyz;
                fls=uLS*wp;
        ...
        // Same fragment shader as TexturedLitShader
        const char* fs = R"(#version 330 core
            in vec3 fn; in vec2 fuv; in vec3 fp; in vec4 fls;
            out vec4 o;
            uniform sampler2D uTex;
            ...
            void main(){
                ...
                vec3 V=normalize(uCamPos-fp);
                int tile=uBaseTile;
                if(abs(N.y)>0.9){ tile+=(N.y>0.0)?0:1; }
                else{ tile+=2; }
                float row=floor(float(tile)/uAtlasCols);
                float col=float(tile)-row*uAtlasCols;
                float ts=1.0/uAtlasCols;
